...
fn ensure_local_folders(
    ...
    // Every folder this account reaches rather than only the ones it owns.
    // Since D-18 an IMAP account owns none of them, so building from
    // `for_account` here makes no rows at all for one and this returns "This
    // account has no folders" on the way in. Each row goes under whoever owns
    // it, and saving is an upsert keyed on the account and the path, so the
    // second account to arrive finds the shared five rather than making a
    // second set.
