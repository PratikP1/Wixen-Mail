...
pushed with a pull request; the merge commit and the pull request's runs are in the report
that closes it. Every spin control in the program names the field a person types in as well
as its arrows, through `names::name_the_spin_control` and its describing twin: the buddy
from `UDM_GETBUDDY` is given the same words through `IAccPropServices::SetHwndPropStr`, and a
reading over 25 spin controls in five built windows holds the field to the arrows on MSAA
and UI Automation. The check interval (1 to 60), Font size (8 to 72) and Default reminder
(0 to 1440) are spin controls holding their own ranges; Mark as read after is Immediately,
After a number of seconds, or Never, with a seconds spin control (1 to 600) reachable only
while a wait is chosen. Page Up and Page Down do nothing in a spin control, measured. EDIT-01
ticked; ledger 408, 409, 410, 412, 413, 414, 419, 420, 421, 422, 424 and 425 fixed, 592 (the
ear) opened.
