...
**Current focus:** Phase 08 (Every number the project quotes) is complete, 08-01 to 08-09 merged, and it was the last phase of the milestone; what is left is a person's, and Current Position says what

## Current Position

Phase: 09 (What the first day of testing found). Current Plan: 11 of 11, which executes first.
**Phase 9 is planned and nothing in it is executed.** Eleven plans in
...
**09-11 executes first, before 09-01**, on #66: the tester's profile path
is two profiles on this machine, chosen by the identity of the process that
launched the program (the plain `%LOCALAPPDATA%\wixen-mail` with the July
settings file, and the copy Windows keeps for launches under the Claude
desktop application's package identity, holding his 2026-09-15 session, the
Gmail account, 12,872 messages and his hand-set English (United States)); the
July file loads and saves in a temp folder with the version stamp copied
through, so a stamp names the build that created a file; and no 2026-09-15
log with content exists anywhere, so the log symptom is real. 09-11 makes the
program say which folder it is in, re-stamps the version on save with a
test through the July file's shape, and holds the log past its second
minute on the release binary; it asks the tester one question, how he
launches. The other ten plans moved down one wave each. Those ten were
nine at first and were corrected the same day after the plan check found
three blockers and twelve warnings (the search index as `strip_markup`'s second
...
plan sixteen files wide split into 09-08 and 09-10; and, first "the
tester's profile is on another machine", then the two-profile finding
above, which replaced it). The phase is planned from the 44 GitHub issues
(#20 to #63) Pratik's first day of testing produced on 2026-09-15 with
`0.125.1+g3e633252`, from #66 filed on 2026-09-16, and from the order he
agreed on 2026-09-16 in seven groups; this phase is the first two, the
version to `1.0.0-alpha.1` (#46, 09-01) and the cause-known defects
an hour to a day each (#66 first, then #21, #32, #44, #56, #36, #39, #42 with
#40 point 5, #33, #51, #53, #34, 09-02 to 09-10). The five later groups are
named in the phase README with their issues and are not planned. Thirteen
requirements,
`FOUND-01` to `FOUND-13`, are in `REQUIREMENTS.md` with evidence re-taken
today; the coverage count is 57. Every file and line the issues cite was
...
04.1 Mail moves between accounts, context only. 05 The other five modules keep up, 8 plans. 05.1 Notes and contacts reach a server, 6 plans. 05.2 Notes in OneNote, 3 plans. 07 Installing, updating and what is stored, 9 plans. 06 was researched and deliberately not planned until the tree it measures existed, then planned 2026-09-12 and executed. 08 the same, planned 2026-09-14 with 9 plans against `main` at `b14d6379`, the last phase of the milestone until 2026-09-16. 09 What the first day of testing found, planned 2026-09-16 with 11 plans (9, then a tenth after the plan check, then an eleventh for #66 that executes first) against `main` at `524ff24f` from the 44 GitHub issues of the first day of manual testing, not from a research document; inserted after phase 8 because the milestone's verification is that testing and its findings are the milestone's work now. Nothing in it is executed.
