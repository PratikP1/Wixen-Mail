...
Phase: 06 (How the application speaks). **06-06 is partial on purpose: task 1
of 2 merged from branch `a-workflow-change-earns-the-checks-that-read-it`,
red `05ec26a4` and green `dd4934fe`, merged into `main` at `9f86ba6f`, still
version 0.123.0. A commit that
...
after. The checkpoint between the tasks asks whether to pin the Axe.Windows
release and how many windows the scan should look at. Both are Pratik's.
The summary at `phases/06-how-the-application-speaks/06-06-SUMMARY.md` sets
out the options, the costs and a recommendation for each, and what task 2
needs to know. `WINDOWS.md` 384 to 387.**
