...
**Current focus:** Phase 04.1 (Mail moves between accounts)

## Current Position

Phase: 04.1 (Mail moves between accounts). IN PROGRESS, 3 of 4 plans done and merged. 04.1-01 a destination is a folder in an account; 04.1-02 a copy that crosses accounts, every account offered, and one branch open; 04.1-03 the move itself, with nothing removed at the source until the destination has answered. `main` is at `f944af7`, version `0.90.0`, and it is **not pushed**.

Current Plan: 3
Total Plans in Phase: 4

**What 04.1-03 leaves for 04.1-04.** Mail moves between accounts now, and the
half decision 17 still owes is the storing half: the raw bytes written before
the append, so a move interrupted by the program closing can be finished without
going back to the source server. The question that makes those bytes actionable
is built and is `mail_across_accounts::whether_the_destination_has_it`, a named
function taking the folder, the identifier and what the folder held before the
send. 04.1-04's resume path calls it from a place where no move is in progress
and has to have stored that before-list beside the bytes, because without it the
question cannot tell a message that arrived from one that was always there.
`docs/privacy.md` is also 04.1-04's: this plan wrote nothing there because it
stores nothing new, and the sentence about what the store leaves on disk belongs
in the same commit as the table it describes. The PIM move still names accounts
its own way, which is phase 5's.

**Three things 04.1-03 found that the next plan should carry.** An ordering
assertion across two servers cannot be made from two transcripts compared after
the fact: they are two lists with no shared clock, so comparing positions is
arithmetic that cannot fail. The witness has to be taken at the moment that
matters, which here means the destination reading the source's transcript from
inside the append. Second, an `APPEND` is not safe to send twice, so it must not
go through `once_more_if_the_connection_went!`; `append_message_once` exists for
that and 04.1-04's resume path has the same hazard in a worse form, because it
will re-append after a restart. Third, `ImapSession::remove_these` is refused in
the words "replace a saved draft", and three callers now avoid it for that
reason rather than one; that is ledger 190.

**Eleven ledger entries are open from this phase, 179 to 190.** 179 is still
worth carrying forward: `move_or_copy_message` and `spawn_folder_move` each ask
`owner_of` with the modal dialog between them. 183 to 185 and 187 to 189 are what
only a real account or a real screen reader settles: no message has crossed
between two live accounts in either direction, the ambiguous append has never
happened against a real server, nobody has heard the window with several accounts
in it, whether Enter chooses in that window was read off the code rather than
pressed, and a folder somebody turned syncing off for is still offered by the
picker with only the changelog saying what that means.

**04.1-03's human checkpoint was not run and is recorded as an unrun
verification**, at Pratik's instruction that manual testing happens later. It is
written out in full in `04.1-03-SUMMARY.md` under its own heading: two real
accounts on different providers for the move itself, and NVDA then Narrator on
the picker with several accounts set up. Two answers stop the phase rather than
continuing it, and the summary says which.

**Owed after the merge, and not on the critical path:**
`scripts/guards.sh --touched-by 611f7cf`, which subsumes 04.1-01's and
04.1-02's.

**`progress.completed_plans` is 62**, counted from the `*-SUMMARY.md` files on
...
Stopped at: Completed 04.1-03-PLAN.md and merged to main at f944af7. Its human checkpoint was not run and is recorded as an unrun verification in 04.1-03-SUMMARY.md.
