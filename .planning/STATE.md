...
Phase: 04.1 (Mail moves between accounts). IN PROGRESS, 2 of 4 plans done and merged. 04.1-01 a destination is a folder in an account; 04.1-02 a copy that crosses accounts, every account offered, and one branch open. `main` is at `8c21a02`, version `0.89.0`, and it is **not pushed**.

Current Plan: 2
Total Plans in Phase: 4

**What 04.1-02 leaves for the plans after it.** Mail does not **move** between
accounts. Copying does. A move is an append and then a removal at the source, and
the removal, with the destination asked whether the upload landed, is 04.1-03;
keeping the raw bytes so recovery does not depend on the server behaving is
04.1-04. Both halves come from decision 17 of 2026-09-07. A move across accounts
is refused in words today, at the moment somebody asks for it. The PIM move still
names accounts its own way, which is phase 5's.

**Three things 04.1-02 found that the next plan should carry.** A session built
from an `Account` reads what this program may do out of the stored settings of
whoever is running the suite, so no test can assert a write through one; the
crossing is behind two traits for that reason and 04.1-03's removal needs the
same treatment. A refusal test passes when its fixture fails earlier for a
different reason, which cost one test here its ability to see its own defect. And
a stub that fails everything reddens more tests and measures less than a stub
that is the plausible wrong answer.

**Seven ledger entries are open from this phase, 179 to 186.** 179 is still worth
carrying forward: `move_or_copy_message` and `spawn_folder_move` each ask
`owner_of` with the modal dialog between them, so a sync that replaced the
message list while the window was open could still leave the second falling back
to the account on screen. 183 to 185 are the three things only a real account or
a real screen reader settles: no message has crossed between two live accounts,
nobody has heard the window with several accounts in it, and whether Enter
chooses in that window was read off the code rather than pressed.

**Owed after the merge, and not on the critical path:**
`scripts/guards.sh --touched-by 611f7cf`, which subsumes 04.1-01's.

**`progress.completed_plans` is 61**, counted from the `*-SUMMARY.md` files on
