...
Phase: 04.1 (Mail moves between accounts). IN PROGRESS, 1 of 4 plans done and merged. 04.1-01 a destination is a folder in an account, one builder feeding the sidebar and the move window, and the All Inboxes defect closed. `main` is at `4e3ae9a`, version `0.88.0`, and it is **not pushed**.

Current Plan: 1
Total Plans in Phase: 4

**What 04.1-01 leaves for the plans after it.** The picker still receives one
account: `where_mail_can_go` answers with every account's branches and
`move_or_copy_message` filters to one, and that filter is the one line 04.1-02
takes out. Mail still does not move between accounts. The PIM move still names
accounts its own way, which is phase 5's.

**Four ledger entries were opened, 179 to 182**, and the first is worth carrying
forward: `move_or_copy_message` and `spawn_folder_move` each ask `owner_of` with
the modal dialog between them, so a sync that replaced the message list while
the window was open could still leave the second falling back to the account on
screen. Passing the first answer would close it and would take
`spawn_folder_move` out of the list `tests/wired.rs` holds to asking `owner_of`,
and that check exists because this fault has now happened four times.

**Owed after the merge, and not on the critical path:**
`scripts/guards.sh --touched-by 611f7cf`.

**`progress.completed_plans` is 60**, counted from the `*-SUMMARY.md` files on
