...
| `b3a36fb2` | green: an address added without its spaces, a birthday outside 1900 to 2100 kept as it was, three records re-measured | `affected`, 154 s, then amended for the changelog line, `docs_only`, 81 s |
| `4006d79f` | red: the Prefix and Suffix boxes offer no ExpandCollapse on UI Automation (2 named) | `red`, 65 s |
| the green after it | green: the two boxes named by their labels, ledger 601 and 602, two records re-measured | |
...
Opened, both halves: 596 (the editor under NVDA), 597 (five name parts through a real account), 598 (the numbering data and pull request 110), 599 (third-party licence notices), 600 (the address dialog's English list); after the pull request's scan, 601 (the Birthday Month list's value element with no name, ledger 411's class) and 602 (ledger 416's class judged). The front matter reads 602 entries, 547 open and 55 fixed. None closed.
...
11. **Two faults found by the pull request's review, fixed after the documents commit.** An address typed with spaces around it passed the shape check and was stored with them; it is now added trimmed (`an_address_to_add` replaces `an_address_refusal`). A stored birthday in a year the year control does not offer, 1815 for one, would have opened clamped to 1900 and been saved so; it is now kept as it was, like a birthday written as words. A third point, that `PhoneEntry.country` needs `serde(default)`, was checked and does not hold: the struct carries `#[serde(default)]`, and every stored list read in the cases has no `country` key. A fourth, about `.githooks/commit-msg`'s comments, is about `main`'s own history and not this branch. The summary and the changelog line on the birthday changed in that green commit, so the documents came in more than one commit.
12. **The pull request's Accessibility scan found three violations in the contact editor.** Run 35995441987, on the tree before the two faults above: the Prefix and Suffix boxes as combo boxes without the ExpandCollapse pattern, and the Birthday Month list's value element focusable with no name. The first two were this plan's: `set_accessible_name` on an editable combo box puts it behind the MSAA proxy on UI Automation, which offers no ExpandCollapse. A reading in the target reproduced it red (`4006d79f`); the boxes are now named by the label before each, which Windows reads on both channels, and the reading is green with every name at the edit still read. That judges ledger 416's class, left open for the event form's Category box (602). The month value element is the class of ledger 411, 415, 418 and 423, in the list `build_date_fields` builds for three windows, and is left for the fix that reaches all of them (601). The phone-number target scanned with 0 violations, and the MSAA walk found no operated element without a name in either window.
