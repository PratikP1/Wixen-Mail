...
provides:
  ...
  - "tests/every_spin_control_names_the_field_a_person_types_in.rs: 14 readings over 25 spin controls in five built windows"
...
tech-stack:
  added: []
  patterns:
    - "A spin control is named on both of its windows: the arrows through the accessible object, the buddy edit through IAccPropServices::SetHwndPropStr, which MSAA and UI Automation both read"
...
**Every spin control in the program now names the field a person types in, on MSAA and UI Automation, and the check interval, Font size, Default reminder and Mark as read after's seconds are spin controls holding their own ranges.**

Started 2026-09-23T21:07Z, finished about 22:10Z; three tasks, six commits on branch `12-06-spin-controls` from `main` at `b00be9c0`.

## What works, measured

A Windows spin control is two windows. `get_handle` answers the arrows, an `msctls_updown32`, which is where `set_accessible_name` attached its object; Tab lands in the arrows' buddy, an `Edit`. Read on the built windows before the change (the red run of `6fe17bff`), 19 spin controls in five windows:
...
What only a person can settle: whether NVDA says each field once with its name, speaks the value after Up and Down, and reads Mark as read after's three entries and the seconds (ledger 592). The twelve scan findings gone from the Accessibility scan is the pull request's run.
...
| this commit | docs: the pages, the ledger, the summary and the four marks | |

## The pull request's runs

Filled in on `main` after the merge.
...
**8. The ledger reasons cite `b439452e`, the green that fixed them, rather than the merge commit.** The documents commit is made on the branch before the pull request, when the merge commit does not exist; the report gives it.

**9. `read_the_calendar_and_pim_page` lost its `base` parameter**, which only the removed parse fallback read.

**Total deviations:** 9, none architectural. The reading covers more than the plan asked and the one plan truth that the toolkit cannot meet is measured and said.

## Tests

| Target | Before | After |
|---|---|---|
| `tests/every_spin_control_names_the_field_a_person_types_in.rs` | new | 14 |
...
T-12-21: `parts` and `from_parts` round-trip immediately, never and waits of 1, 2, 45 and 600, and a stored 5000 shows as 600, never as Immediately; the two settings guards are green at 68. T-12-22: a zero buddy is said at debug and the arrows keep their name; the reading counts the spin controls it finds. T-12-23: the reading asserts each field's name equals its own arrows' on 25 spin controls. T-12-SC: no crate added, three features of the `windows` crate already in the tree.
...
408, 409, 410, 412, 413, 414, 419, 420, 421, 422, 424 and 425 fixed in both halves with `resolved_at` 2026-09-23T21:30:21Z. 411, 415, 418 and 423 are the AM or PM lists' and the Send on Month list's value elements, not spin controls, and 416 and 417 are the Category combo box and the form's height: all six stay open, not this plan's. 592 opened for the ear. The header reads `open_count` 538 and `fixed_count` 54, eleven lower and twelve higher than 549 and 42.

## Exception set

Nothing tracked was rewritten by script. `cargo fmt` formatted Rust files; `scripts/guards.sh` applied and restored its own breaks in three runs, as the project's tool does. The one probe, `tests/zz_probe_spin_keys.rs`, was written with the Write tool, run once, and deleted with `rm`; it was never tracked. Every other edit was by hand.

## Self-Check: PASSED

Files: the target, the summary, and every modified file above exist. Commits `6fe17bff`, `b439452e`, `f022934d`, `42f4213f`, `b331b1e9` are on the branch.
