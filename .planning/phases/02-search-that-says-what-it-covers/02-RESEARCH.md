...
`CLAUDE.md`, which does not contain the phrase. Widening `Allowed` to cover reads touches
...
index goes stale rather than the coverage narrowing, which is the opposite of what SEARCH-02 assumes.
...
| Sentence describing what a search asks (D-2-04) | `application::saved_searches` | none | Every other sentence this module owns lives here (`a_typed_search_in_words`, `what_a_search_found`, `a_row_for`). A second sentence builder in the window layer is the drift `EXPERIMENTAL_WARNING`'s comment records. |
...
The third being unwritten is already recorded as a known defect, and the record is itself checked by
...
Note also `allowed_per_account: HashMap<String, Allowed>`, where the same nested `Allowed` appears as map
...
- **Every `may_i` call site.** All sixteen are writes. Fourteen in
  ...
  **`02-CONTEXT.md` names only `imap.rs`. `pop3.rs` has a `may_i` too**, with the same one-line body
  calling `outward::permitted`, so any structural guard written about `may_i` must see both files or
  it measures half of what it claims to.
...
It builds four buttons (`&Add...`, `&Edit...`, `&Delete` and `&Close`, at
...
layer with nothing checking it. The fix is one line each, building both `Vec<String>` from the
...
| `b.message_id = m.id` | `message_bodies.message_id INTEGER PRIMARY KEY` is a rowid alias, so the join is a rowid seek and costs nothing | `src/data/message_cache/mod.rs:2046` |
...
narrowing as more bodies are evicted". The narrowing is real for the saved-search path and
...
| Make the saved-search body scan read the FTS index rather than `message_bodies` | Not possible: `content=''` means the FTS table stores no retrievable content, so you cannot select `body` back out of it | nothing |
...
test module, at `src/presentation/wx_app.rs:6539`. Its join field, verbatim
...
2. `selects`'s own doc records the empty-question-list trap that `All` creates, verbatim
   ...
   ```
   The rule editor makes an empty condition list reachable for the first time too, because a user can
...
its own stored spelling, and it must not collide with either, which is D-25's keying rule.
...
must take a real serialised object and *remove the key*, which is the shape at `config.rs:1010-1022`.
...
either way, because it compares `default()` to `NOTHING` and so cannot see them being wrong together).
...
|----------|-------------|-----------------|
| Stored data | `app_config.json`'s `allowed_changes` object gains a key; `allowed_per_account`'s map values likewise. `saved_searches` and `saved_search_questions` tables unchanged, because D-2-03 and D-2-01 need no schema work, confirmed against `mod.rs:1590` and `:1615`. | Code edit only: a serde default. **No data migration.** An existing file with two keys must go on parsing, and the third answers `true`. |
...
| Build artifacts | None. No package name changes and no `Cargo.toml` change, because this phase adds no dependency. | None |
...
|------------|------------|-----------|---------|----------|
| `wxdragon` | Rule editor | ✓ | `=0.9.17` (pinned, with `aui`, `richtext`, `webview`) | none |
| `rusqlite` / SQLite FTS5 | Search, counts | ✓ | In use; `message_search` and `calendar_search` both exist | none |
| `flate2` | Body unpacking | ✓ | In use at `Compression::new(6)` | none |
| `serde` / `serde_json` | `Allowed`, `AppConfig` | ✓ | In use | none |
| A live IMAP account | The body fetch | ✗ | not applicable | **No fallback, and none is wanted.** See below. |
...
| Screen reader listening | Announcements are distinguishable in practice | manual | none | Pratik's, and his to schedule |
...
| V5 Input Validation | **yes** | The rule editor's Pattern box reaches `RegexBuilder` when match type is `regex`, and reaches SQL nowhere, because `FilterEngine::matches` compares in Rust. The FTS path is separate and already handles this: `as_a_search` quotes every word (`searching.rs:129-131`), which its doc calls "the whole of the safety here". |
...
**Example**, at `src/presentation/wx_managers.rs:2545`, `2707`:
```rust
...
**Example**, at `src/presentation/wx_managers.rs:2666-2676`, verbatim
...
**Example**, at `src/presentation/wx_settings.rs:1502-1505`:
```rust
...
**Example**, at `src/presentation/wx_settings.rs:1821-1829`, the feedback channels loop, whose comment
...
- `wxdragon-0.9.17` widget file listing from the cargo registry. This establishes what controls
  exist, not their exact APIs.
...
- Question 1 (the three places, the two traps): HIGH. Every claim is quoted from source read this
