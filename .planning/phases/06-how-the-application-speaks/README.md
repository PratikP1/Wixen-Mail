...
| 06-06 | 6 | 3 | none | decision, open | **Task 1 done, task 2 waiting on the checkpoint.** A change under `.github/workflows/` earns every check, measured on a staged break before and after. Whether to pin the scanner and how many windows it looks at are Pratik's; the summary sets out both with a recommendation. The tree says at least fourteen windows are outside the scan, not nine |
