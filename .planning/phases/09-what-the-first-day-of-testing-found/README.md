...
**#66, filed and withdrawn on 2026-09-16.** A read of the tester's profile
through a shell started from this harness found a July settings file and an
empty log, and #66 said settings were never saved and the log stopped. Read
through a plain Win32 process the profile was written on 2026-09-15 with his
hand-set language, and its `log_level` is `error`, which is why the day's
log is empty: no settings defect and no log defect. A plan 09-11 was written
for it and removed the same day. Its one real finding, that a settings file's
version stamp names the build that created the profile and not the one that
last wrote it, is a `[D]` line of FOUND-01 and a task in 09-01.
...
|---|---|---|---|---|---|
| 09-01 | 1 | 1 | #46 | closes, tree side | `Cargo.toml` to `1.0.0-alpha.1`; a pin naming the step; the installer's four-field version held above today's; a saved settings file re-stamped with the writing build (the one item of the withdrawn #66); a Release level that publishes the version as it stands; the rule for moving inside a prerelease in `CLAUDE.md`, the changelog, `BETA_RELEASE.md` and the release skill |
...
shape or a file set: the version alone, with the settings stamp beside it;
two stored-value fixes; two things about Undo Send; two things about sort;
the checkbox class with its scan targets; the tab-row diagnosis; the reading
path; the import reader and the Save As writer; the measurement; and the
folder picker with the documents, which the plan check cut out of the import
plan on 2026-09-16 because sixteen files in one plan was the widest in the
phase and the documents share no code with the reader. Each is two or three
tasks.
...
| #21 | the tester's profile cannot be read from here; then, read through this harness's shell, it is a July file stamped alpha 22 with a bare `en` | read through a plain Win32 process (Python), `%LOCALAPPDATA%\wixen-mail` is his live profile: `app_config.json` 1,921 bytes, 53 keys, written 2026-09-15 18:39, stamped `0.7.7` (the build that created the profile; `save` copies the stamp through), `"language": "en-US"` set by hand and stored, `log_level` `error`, beside a 20 MB database with his Gmail account and 12,872 messages. The profile was created by 0.7.7 before the default changed, so it stored the bare `en` that lands on Caribbean; the cause stands. bash and PowerShell started from this harness read, at the same path, a stale 886-byte July copy under `Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local\wixen-mail` | `python -c "import json; d=json.load(open(r'C:\Users\prati\AppData\Local\wixen-mail\config\app_config.json')); print(d['version'], d['language'], d['log_level'], len(d))"` -> `0.7.7 en-US error 53`; `stat -c %s` on the same path from bash -> 886 |
| #66 (withdrawn) | settings never written since alpha 22; the log stops after two minutes or stays empty | the read behind the issue was the stale copy; his settings are saved and his log level is `error`, so the day's log is empty. The stamp is not a proof of which build wrote a file: `version` is set in `AppConfig::default()` (`config.rs:651`) and copied through by `save()`, shown by a scratch test that saved a file stamped alpha 22 from this build and read the stamp back unchanged with 56 keys. That one item is FOUND-01's last `[D]` line | the scratch test in `config.rs`, run with `--nocapture`, removed with `git checkout` |

The last five rows before #21's were found by the plan check on
2026-09-16, after the first draft of these plans; each plan's
`<premise_corrections>` says so where it was wrong. The #21 and #66 rows
were settled the same day after three wrong readings of one file.
...
- bash and PowerShell started from this harness read a stale July copy of
  `%LOCALAPPDATA%\wixen-mail` at that path, kept under
  `Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local\wixen-mail` (886 bytes and
  alpha 22 where the live file is 1,921 bytes and 0.7.7; both readers answer
  `GetCurrentPackageFamilyName` 15700, no package, so the identity call does
  not tell them apart and the bytes do). An executor reads the tester's
  profile only through Python from here, and starts the release binary only
  against a temp folder pinned with `WIXEN_MAIL_DATA`, as 08-03's harness
  does, so no run touches his profile and no reading is of the copy.
