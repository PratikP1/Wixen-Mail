...
**Plans**: twenty-one, one per wave, listed in `phases/11-reading-and-the-list/README.md`,
...
One per wave because sixteen of the twenty-one write `src/presentation/wx_app.rs`, every one
writes `docs/changelog.md` and all but the closing read write `guards/guards.toml` under the
...
after, #30 with #27's thread, #31, #26, #77 with #81 beside it, #82 beside that, #62, #28
with #29, then #80 in two plans and #79 after them), then the status sentences read once all
of them are written, then the documents. Each plan ends with the `gh issue close` or `gh issue comment` the executor
