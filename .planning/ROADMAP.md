...
**Plans**: 1/13 plans executed, one per wave. Two shared files, `guards/guards.toml` and `docs/changelog.md`, are touched by most plans under the same-commit rules, and `src/presentation/wx_app.rs` by most, so the plans are ordered rather than run in parallel.
...
|-------|----------------|--------|-----------|
| 1. Folders and conversations | 1/13 | In Progress|  |
