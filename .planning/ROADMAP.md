...
**Success Criteria** (what must be TRUE):
  ...
  5. One whole-tree guard sweep completes, `scripts/guards.sh` unfiltered over every record in `guards/guards.toml`, and each record it reports short is corrected by hand and then re-measured. This is the one sweep of the milestone: by the decision of 2026-09-03 no sweep runs per merge or per phase, so nothing before this point has re-measured a record that only the whole sweep can reach. Expect about 19 hours and expect findings, since the tree will be many phases past the changes being judged. The 19 hours is 783 records, counted 2026-09-14 at `b14d6379` by a TOML reader over `guards/guards.toml`, times about 86 seconds a record, being about 35 seconds to rebuild and 51 seconds for the library run at eight threads over 7,244 tests, one record measured 2026-09-14 on the same commit through `scripts/guards.sh` in 138 seconds including its 51-second pre-read; this line said "roughly 15 hours" until 2026-09-14, and 08-01 re-takes both terms before the sweep is scheduled.
...
- [ ] 08-01-PLAN.md, every count and rate the phase depends on taken today by its command, `docs/development/measurements.md` created to hold them, a reading that refuses a row without its command, date and commit, the sweep rate timed, the mutation timeout calibration re-derived. The mutant count is 12,335 by `cargo mutants --list`, which is not "about two days" at any rate this machine gives.
- [ ] 08-02-PLAN.md, criterion 3 as checks: a count on a product page carries a date and a source; the three test-count pages quote one row; the red/green ratio is computed and printed rather than written at five sites; twelve prose figures are held to the constants they restate.
- [ ] 08-03-PLAN.md, PERF-01, PERF-02 and PERF-04: a usable line the application writes once, a profile of exactly 1,000 messages of a defined shape, a harness that starts the release binary and reads the working set of the process and its WebView2 tree, the three numbers taken with the date, machine and build.
- [ ] 08-04-PLAN.md, PERF-03: sort, filter and scroll timed over 200,000 rows by a harness kept in the tree; the virtual text callback's body pulled into a function over slices with a reading that holds it to naming no database; the generator and the sort moved where tests and the mutation tool reach them.
- [ ] 08-05-PLAN.md, PERF-05: coverage re-measured with the 2026-07-26 command and the low areas attributed to the untested transport with their own figures.
- [ ] 08-06-PLAN.md, corrections by hand: the four sweep-cost figures retired, the awk replaced by the parser, comments in source and scripts, the three planning documents with dated figures.
...
- [ ] 08-08-PLAN.md, PERF-07 and criterion 4: shards on one commit; the rate measured on one shard under both suite shapes; which run to make is Pratik's with the products in front of him; the report read after the last process exits, survivors killed or reasoned.
...
| 8. Every number the project quotes | 0/9 | Planned 2026-09-14, not started | - |
