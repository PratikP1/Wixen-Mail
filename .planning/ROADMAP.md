...
**Requirements**: LIST-01, LIST-02, LIST-03, LIST-04, LIST-05, LIST-06, LIST-07, LIST-08, LIST-09, LIST-10, LIST-11, LIST-12, LIST-13, LIST-14, LIST-15, LIST-16, LIST-17, LIST-18, LIST-19, FOUND-17, FOUND-18
**Success Criteria** (what must be TRUE):
...
**Plans**: twenty, one per wave, listed in `phases/11-reading-and-the-list/README.md`,
...
for #75), and five inserted that evening against `eb5d8517` for six issues filed that
evening, two of which became a third task of a plan not yet executed (#83 in 11-06.1, #81 in
...
own). One per wave because fifteen of the twenty write `src/presentation/wx_app.rs`, every
one writes `docs/changelog.md` and all but the closing read write `guards/guards.toml` under
the same-commit rules, and a wave is a set of plans sharing no file. The order: the red CI,
the NVDA workflow's verdict, then Pratik's order for the group (#70, #71, #84 beside it,
#25, #27's label and key, #76 with #83 before the selection it lands after, #30 with #27's
thread, #31, #26, #77 with #81 beside it, #82 beside that, #62, #28 with #29, then #80 in two
plans and #79 after them), then the status sentences read once all of them are written, then
...
- [ ] 11-12-PLAN.md: The pages, the listening lines, and the phase's closing read (all nineteen)
