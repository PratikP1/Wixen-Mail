...
**Requirements**: LIST-01, LIST-02, LIST-03, LIST-04, LIST-05, LIST-06, LIST-07, LIST-08, LIST-09, LIST-10, LIST-11, LIST-12, LIST-13, FOUND-17, FOUND-18
**Success Criteria** (what must be TRUE):
...
**Plans**: fifteen, one per wave, listed in `phases/11-reading-and-the-list/README.md`,
twelve planned 2026-09-18 against `main` at `744d05ef` and three inserted later that day
against `08197657` for issues filed that afternoon (11-06.1 for #76, 11-09.1 for #77, 11-13
for #75). One per wave because thirteen of the fifteen write `src/presentation/wx_app.rs`
and every one but the first writes `docs/changelog.md` and `guards/guards.toml` under the
same-commit rules, and a wave is a set of plans sharing no file. The order: the red CI, the
NVDA workflow's verdict, then Pratik's order for the group (#70, #71, #25, #27's label and
key, #76 before the selection it lands after, #30 with #27's thread, #31, #26, #77 beside
it, #62, #28 with #29), then the status sentences read once all of them are written, then
the documents. Each plan ends with the `gh issue close` or `gh issue comment` the executor
...
- [ ] 11-06.1-PLAN.md: After a delete the cursor lands on the next message on the control itself, and a re-read keeps it on the same message by id (#76)
...
- [ ] 11-09.1-PLAN.md: Attachment said once on landing: the earcon on by default, the spoken event off, the column kept (#77)
...
- [ ] 11-12-PLAN.md: The pages, the listening lines, and the phase's closing read (all thirteen)
