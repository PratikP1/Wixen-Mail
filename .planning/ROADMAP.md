...
| 7. Installing, updating and what is stored | 7/9 | In Progress| 07-01 to 07-05 merged; criteria 3 and 6 close, neither heard; criterion 2 has everything but applying, and nothing it can say has met a published release. 07-06 merged one task of three and its summary says `partial`: criterion 5 needs a workflow run nobody has made, so SHIP-05 is still either two CI jobs or a port. 07-07 closes the half of criterion 1 that is about this repository rather than a certificate authority, and rewrites criterion 2 to cover the download; criterion 1 itself waits on an Azure account. The count is of summary files on disk, which is what the check that reads this row counts, so it is not a count of plans that closed what they were written for |
