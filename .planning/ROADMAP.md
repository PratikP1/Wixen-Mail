...
- [ ] 12-02-PLAN.md: The separate window as a process of its own with a WebView2 profile of its own, the route reaching it, the erase reaching the profile, the two next-build sentences retired (#80 second half, LIST-19, ledger 566; moved from phase 11's 11-11.2)
...
| 12. The editors, and what the alpha still owes | 1/12 | Planned 2026-09-20 against `main` at `0ad66e48`: twelve plans, one per wave, two moved from phase 11 (12-02 the separate window, 12-03 the status sentences) with their premises re-taken, ten written that day; 12-01 first for NVDA run 35520201976's red case. 12-01 merged 2026-09-22 at `e29c514b`, criterion 1 closed structurally: the page window gives the keyboard back to the browser whenever it is activated, measured on a built window with the reading taken red by hand, so no handler was added and the run's failure was the case's return; the case waits for the front and writes down where its next key goes, and its green is the next push of `main` (ledger 567). 1,030 records by the TOML reader, census 798 swept and 232 since, ledger 567 with 532 open and 35 fixed, 8,653 tests on the last whole gate, `main` pushed at `0ad66e48` and 6 commits ahead of it by `git rev-list origin/main..HEAD --count` before this summary lands. 12-02 is next | In Progress |
