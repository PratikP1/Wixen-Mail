...
- [ ] 08-01-PLAN.md, every count and rate the phase depends on taken today by its command, `docs/measurements.md` created to hold them, a reading that refuses a row without its command, date and commit, the sweep rate timed, the mutation timeout calibration re-derived. The mutant count is 12,335 by `cargo mutants --list`, which is not "about two days" at any rate this machine gives.
