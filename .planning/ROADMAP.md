...
it, and `main` has not been pushed since `6cb8f17c` (ledger 609).
...
#55, #57, #59), then the automation (#58, #60, #61), then #53's remaining halves.
...
**Plans**: not planned yet, 2026-09-20. Written as an entry and requirements only, because
plans written now would rot against a tree phase 12 changes (the composer, the editors, the
menus, the settings screen and the records every one of these would touch). No progress row
until the phase is planned, since a row needs a directory on disk.
...
2026-09-20 after phase 11 did, with phases 13 and 14 written as entries the same day.
