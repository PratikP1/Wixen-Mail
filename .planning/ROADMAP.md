...
- [ ] **Phase 9: What the first day of testing found** - The version becomes `1.0.0-alpha.1`, and twelve cause-known defects from the first day of manual testing are fixed test-first, each an hour to a day
...
**Success Criteria** (what must be TRUE):
  ...
  1. `Cargo.toml` says `1.0.0-alpha.1`, `--version` prints it, `common::version` has a test
     ...
     the tree carries without bumping it first, and `CLAUDE.md`, the changelog's opening
     paragraph, `docs/BETA_RELEASE.md` and the release skill all state the same rule for how a
     version moves inside a prerelease. Whether the first alpha is published is a dispatch and
     is Pratik's. (#46)
  ...
  10. The program says which folder it is reading and writing in its log's first lines and on
     the settings status line; a setting confirmed on the Settings screen is on disk with the
     running build's version stamp, proved through the July file's exact shape; the status line
     says the path written or the reason it was not, at a priority a person hears; and a
     release binary run against a temp profile for longer than two minutes writes a line in
     its third minute, proved by a harness on the 08-03 launcher. This one executes first.
     (#66, added 2026-09-16)
...
**Plans**: eleven, one per wave, listed in `phases/09-what-the-first-day-of-testing-found/README.md`,
planned 2026-09-16 against `main` at `524ff24f`; nine when first written that day, a tenth
cut out of the import plan after the plan check found it sixteen files wide, and an eleventh
added for #66 the same day. One per wave because every plan writes `docs/changelog.md` and
nine of the eleven write `guards/guards.toml` under the same-commit rules, and a wave is a set
of plans sharing no file. **09-11 executes first, before 09-01**: nothing the tester reports
about a setting can be trusted until one survives a restart, and #22 cannot be read until the
log holds something. Each plan ends with the `gh issue close` or `gh issue comment` the
executor runs after the merge, quoting the merge commit, so the tracker and the tree agree;
closing an issue is not a publish.

- [ ] 09-11-PLAN.md: First. The program says which folder it is in, a setting survives a restart with the build's stamp, and the log outlives its second minute (#66)
- [ ] 09-01-PLAN.md: The version is `1.0.0-alpha.1`, the workflow can publish it as it stands, and the rule for moving inside a prerelease is written where the old rule was (#46)
- [ ] 09-02-PLAN.md: A bare stored language resolves to this machine's region and the screen shows what will be used; a snippet is words and never a stylesheet, stored ones put right once (#21, #32)
- [ ] 09-03-PLAN.md: Undo Send on the Edit menu, and a held meeting answer that says it is held (#44, #56)
- [ ] 09-04-PLAN.md: The two sort controls together, a compose setting on the Compose tab, and one sort checked (#36, #39)
- [ ] 09-05-PLAN.md: The five editors reached by the scan, every checkbox named, the empty spacers gone, and the label check widened (#42, #40 point 5)
- [ ] 09-06-PLAN.md: The Settings tab row's events captured, the second one stopped, and a real NVDA holding each tab to once (#33)
- [ ] 09-07-PLAN.md: One reading path for PGP, the S/MIME envelope and the signature bar, on all six surfaces including the whole-conversation reader and the preview pane (#51)
- [ ] 09-08-PLAN.md: The Outlook data file reader wired into the picker, and Save As writing the list's message (#53, points 1 and 2)
- [ ] 09-09-PLAN.md: Settings measured, then opened at once (#34)
- [ ] 09-10-PLAN.md: A folder picker on its own item, the four documents corrected, the guide, and the phase's closing read (#53, points 3 and 7)
...
Save command; and the tester's profile path is two profiles, the plain one holding the stored
bare `en` from July and a second one, kept by Windows for launches under a packaged
application's identity, holding his 2026-09-15 session and his hand-set English (United
States), which is #66's finding and 09-11's premise 1. The five later groups of Pratik's order
are not here and are named in the README.
...
| 9. What the first day of testing found | 0/11 | Planned 2026-09-16 against `main` at `524ff24f`, corrected the same day after the plan check (three blockers, twelve warnings, one plan split in two), then an eleventh plan for #66 that executes first; nothing executed | - |
