...
- [ ] 12-01-PLAN.md: The NVDA case's second half diagnosed from the run's own record, the product half measured on a built page window, the case coming back with Alt+Tab and waiting for the foreground and recording what has focus (run 35520201976, FOUND-20)
...
| 12. The editors, and what the alpha still owes | 0/12 | Planned 2026-09-20 against `main` at `0ad66e48`, nothing executed: twelve plans, one per wave, two moved from phase 11 (12-02 the separate window, 12-03 the status sentences) with their premises re-taken, ten written that day; 12-01 first for NVDA run 35520201976's red case. 1,029 records by the TOML reader, census 798 swept and 231 since, ledger 566 with 531 open and 35 fixed, 8,651 tests on the last whole gate, `main` pushed. 12-01 is next | - |
