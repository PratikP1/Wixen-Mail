...
- [x] **FOUND-22**: What an NVDA case waits to hear is something the program says, and a
  ...
  - [S] The two cases green on the runner at the next run of the NVDA workflow, whose
    ...
    Ledger 576 is fixed.
- [x] **FOUND-23**: The commit gate spends only what a change earns and says where its time
  ...
  - [x] [D] The closing plan of phase 12 runs `scripts/check.sh all` once, by hand, before its
    ...
    **Met 2026-09-24 at `566116d3`**, on branch `12-12-phase-close` with `git rev-list
    HEAD..main --count` at 0: exit 0 after 516 s, read directly from the run, 8,985 tests
    passed and none failed, green on its first run, so no pair was needed. Ledger 585 fixed;
    the row is on `docs/development/measurements.md`. With this line the box is ticked, the
    other seven carrying 12-03.2's names.
  - [S] Whether moving the full gate to the phase's close lets a break through that the
    ...
    **Phase 12's answer, 2026-09-24:** nothing. The nine merges from 12-03.2 to 12-11 went
    in without the whole suite, and the whole suite, the release build and the audit then
    passed on the tree they made, 8,985 tests in 289 s of a 516 s run. One phase is one
    reading; each later phase's closing gate is the next.
...
FOUND-23's box waited for its last `[D]` line, 12-12's task 3, and was ticked when that full
gate came back green at `566116d3`, so all fourteen stand. No `[S]` line was touched but
FOUND-23's, which gained the gate's answer, and
...
- [x] **GAP-02**: Edit, Undo and Redo act on the focused text everywhere, and then on actions
  ...
  - Evidence: `gh issue view 47 --json state` on 2026-09-20: open; the composer's own
    ...
    into a real box and walks them back and forward, and presses Ctrl+Z in the box. 13-06 on
    2026-09-25: every dialog's boxes keep the same history, the editable combo boxes too, a
    password box none, a value a dialog opens holding where its history starts; the number
    fields and wxWidgets' two text-entry dialogs keep Windows' step, named (ledger 621).
    `cargo test --test every_text_box_keeps_a_history`, 10 tests, holds every `TextCtrl` and
    `ComboBox` builder under `src/presentation` to `keep_a_history`, and
    `several_steps_come_back`, 16 tests, reads a combo box, a box in a dialog, Check Spelling
    and a stored contact on real controls. 13-07 on 2026-09-25: in the message list, Undo and
    Redo take back and do again the last mark, star or label, each message as it was, named on
    the Edit menu with the list focused, one step with no timer, sent to the server the way the
    action was and said experimental in Undo's help; the mark after the reading wait replaces
    nothing. `cargo test --lib application::undoing::`, 8 tests, holds the decisions and the
    ...
    (ledger 625). 13-09 on 2026-09-25: Undo and Redo in Contacts, Calendar, Reminders, Tasks
    and Notes take back and do again the last Mark Done, Pin, move, copy or delete with the
    item named, one step shared with the message list: a deletion its account has not been
    told of taken back with its row in one transaction and never sent, one the account has
    taken made again as new and said so, refused while that account's sync runs. `cargo test
    --lib application::undoing::`, 30 tests, among them
    `test_undoing_a_deleted_task_puts_back_every_field_it_had` and
    `test_undoing_a_delete_the_provider_took_makes_it_again_and_says_so`; `cargo test --lib
    data::message_cache::taking_back::`, 10 tests, on a temporary store, among them
    `test_a_restore_that_fails_leaves_the_row_and_the_note_as_they_were`; `cargo test --test
    undoing_a_mark_names_the_message`, 21 tests, reads `pim_command`'s recording, the undo's
    wiring and the four syncs' count. Nothing of it has met a real account (ledger 628).
...
- [ ] **GAP-04**: The reader shows and announces a meeting invitation with its answers, and a
  ...
  - 13-10 on 2026-09-26: a message carrying an invitation, a cancellation or an answer says
    ...
    an `application/ics` part is found. `cargo test --test
    an_invitation_is_said_before_the_body`, 10 tests, two whole raw messages through
    `mime::parse`, the store and `for_message`, among them
    `test_a_raw_invitation_is_said_at_the_top_of_the_bar_and_the_body` and
    `test_the_formatted_window_and_the_preview_say_the_invitation_before_the_message`;
    `cargo test --lib application::invitations::`, 48 tests. No real organiser's
    invitation has been read (ledger 631).
  - 13-11 on 2026-09-26: the `[D]` line's "with Accept, Tentative and Decline" built as
    three native buttons in the text reader's tab and the formatted window, named and
    described by what pressing each will do, on Alt+C, Alt+T and Alt+D, none for an
    invitation that cannot be answered and the reason in the bar, and the three answers on
    the message list's context menu; held by `cargo test --test
    the_invitation_is_answered_from_the_reader`, 12 tests, among them
    `test_an_answerable_invitation_has_three_named_described_buttons` and
    `test_the_formatted_window_has_the_three_buttons_after_the_page`, read over MSAA at each
    button's own handle. "The answer sent through the outbox and the gate": answering takes
    the message it was pressed on and goes through `whether_it_can_be_answered`, whose
    sending gate `application::answering::invitations_from_strangers::test_nothing_is_ever_answerable_with_sending_switched_off`
    holds, into the outbox threaded under the invitation,
    `application::answering::tests::test_the_answer_goes_out_threaded_under_the_invitation`;
    the answer given is kept beside the version,
    `data::message_cache::calendar::tests::test_the_answer_given_is_remembered_beside_the_version`.
    Nobody has heard the buttons (ledger 634) and no reply has reached a real organiser
    (ledger 631). The box waits for 13-13.
...
| FOUND-23 | Phase 12 | Complete 2026-09-24; until then this row read "Pending": 12-03.2 planned 2026-09-23 against `481a7918` from the measurement of nine executors; its last `[D]` line is 12-12's task 3, and its `[S]` line is that full gate and the stage lines the later plans print. 12-03.2 done 2026-09-23 on its seven `[D]` lines; the box and this row stay open for 12-12's task 3. Complete 2026-09-24: 12-12's full gate at `566116d3`, exit 0 after 516 s with 8,985 tests passed, green on its first run; ledger 585 fixed |
...
| GAP-04 | Phase 13 | In progress 2026-09-26: the first `[D]` clause's "invitation's part shown and said before the body" built by 13-10, held by `tests/an_invitation_is_said_before_the_body.rs` and `application::invitations`'s cases; "with Accept, Tentative and Decline" and "the answer sent through the outbox and the gate" built by 13-11, held by `tests/the_invitation_is_answered_from_the_reader.rs` and `application::answering`'s cases; the UID and the calendar changes wait for 13-12 and 13-13, and 13-13 ticks it. Until 13-11 "the answer buttons, the UID and the calendar changes wait for 13-11 to 13-13, and 13-13 ticks it". Until 13-10 "Planned 2026-09-24: 13-10 to 13-13, 13-13 ticks it; not built", and before that "Not planned, 2026-09-20" |
