...
**Filesystem isolation:** the `tempfile` crate. `tempfile::tempdir()` is used wherever a test needs a real filesystem path instead of the user's real directories (`src/application/attaching.rs`, `src/application/mail_sync.rs`, `tests/integration_tests.rs`).
...
- `tests/checkbox_labels.rs`, `tests/flag_names.rs`, `tests/theme_reach.rs`, `tests/docs_links.rs`, `tests/command_line_output.rs` are narrower instances of the same family. Each reads source or docs text and asserts that a specific promise (a label, a flag name, a theme color, a documented link, a CLI message) actually holds in the tree.
...
**Naming:** test function names are full sentences describing the behavior under test, not `test_foo_case1` style. For example: `test_a_group_lives_in_storage_and_resolves_to_a_to_line`, `test_every_handled_command_has_something_that_raises_it`, `test_no_dashes_that_should_be_punctuation`. Follow this naming convention for new tests: the name alone should tell a reader what broke, without opening the file.
