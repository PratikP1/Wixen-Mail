...
| A pin and a server subscription are two questions, so neither overrules the other. Pinning never writes a subscription, and a subscription changing never adds or removes a pin | A pin says where a folder sits in this program's tree on this computer; a subscription says which mailboxes an account asks its server to list, and is shared with every other client. FOLDER-03 forbids pinning reaching a server, and letting a change made in another program rearrange this tree is that program editing somebody's sidebar. Joining the two is a setting somebody turns on, gated like every other server write | Recorded before the storage shape was fixed, as FOLDER-03 requires. Stated in full in `src/application/favourites.rs` |
| Favourites are stored against `(account_id, path)` with both cascades, not against a folder's row id | It is D-25's stable identity, the pair `folders` is unique on, and the pair `set_subscribed` names a mailbox by, so `folders.subscribed` is one join away and the server half needs no migration. A rename rewrites a folder's path, so `ON UPDATE CASCADE` is what makes D-32's "a rename keeps the pin" true; `ON DELETE CASCADE` is the other half. Both are measured by guard records rather than assumed | Good |
