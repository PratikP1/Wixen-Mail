...
| Forward | `Ctrl+L` |
| Search whichever module is showing | `Ctrl+F` |
...
| Print this tab | `Ctrl+P` | Prints the message, the conversation or the attachment in this tab through Windows' own print dialog, with every date written in full. The letter is `P` on the reader's File menu |
...
Where a link opens is a setting, since 2026-09-20: Settings, Reading, "Open
links", which is the default browser unless you change it, the message view,
or a separate Wixen Mail window. These keys work the same in the preview pane
and in the formatted message window a message opens into.
...
| Move to | `Ctrl+Shift+V` | Action menu. In Mail, put this message in another folder, on this account or on another one, opening on the last folder you filed into. In the other modules, move the chosen event, task or note to another calendar, list or folder. On a contact, ask which group it is coming out of and which it is going into. On a reminder, ask which account it should go to |
| Copy to | `Ctrl+Shift+Y` | Action menu, Copy to, Somewhere Else. In Mail, put a copy of this message in another folder, on this account or on another one. In the other modules, put a second copy of the chosen event, task or note in another calendar, list or folder, leaving the first where it is. On a contact, put it in a second group and leave it in the ones it is already in. On a reminder, put a second reminder in another account, or in the same one |
...
| Print | `Ctrl+P` | Print the message you are on in the message list, through Windows' own print dialog, where you choose the printer, the copies and the pages. Its header lines and its words are printed, not its pictures or formatting. On a conversation's row it prints the whole conversation, every message in order, each headed with its date in full. The reader window prints its tab the same way. In Contacts, Calendar, Tasks, Notes and Reminders it prints the item you are on, with the fields `Shift+Space` reads, one to a line. The formatted conversation window does not print in this build |
...
**Moving between misspellings without the dialog.** `Alt+F7` moves the caret to
the next misspelled word and leaves it there, so the next thing you do is type.
It is the key Word uses for the same thing. `Alt+Shift+F7` goes the other way,
so overshooting is one key press to undo rather than a walk through the whole
message again.

| Action | Shortcut | Description |
|--------|----------|-------------|
| Next misspelling | `Alt+F7` | Move the caret to the next misspelled word and say what it could be instead |
| Previous misspelling | `Alt+Shift+F7` | The same, going back towards the start of the message |
...
altogether, so you know the list did not stop where the speaking did. It says
the same thing whichever direction you arrived from. `F7` and its dialog are
unchanged, and nothing opens.

At either end of the message it says there are no more that way and moves
nothing, rather than starting again at the other end without telling you. In a
message with nothing wrong it says that.

Neither key is affected by anything you told `F7` to ignore. An ignore lasts for
the pass you made it in, and there is no Ignore to press here, so a word you
passed over in the dialog is still a word these keys will take you to.
