...
- **Numbers are spin controls, and every spin control's typing field has a name.** The tester
  ...
  chosen from the old list of seven is kept. The server ports stay typed. And every spin
  control in the program, in Settings, the account editor, Edit Event, Send Later and Insert
  Table, now carries its name on the field a person types in as well as on the arrows beside
  it, on both of the channels screen readers read. Before this, NVDA heard ten of those fields
  with no name at all and nine by whatever label sat before them, found by the accessibility
  scan in September (ledger 408 to 425). A spin control's description, such as the check
  interval's sentence, reaches the field too.
  Known limitations: nobody has heard a spin control's field name with a screen reader since
  this change; the reading is over MSAA and UI Automation in a test. `Page Up` and `Page Down`
  do nothing in a spin control.
...
- **Mark as read after applies the moment Settings is saved.** The tester on 2026-09-20, on
  ...
  whole message, not after Shift+Space, whichever of its values he chose. The shape of it: a
  setting changed in Settings did not apply until the program was started again. The rule
  was right; the wait it was asked with was read once, where the main window was built, and
...
- **Ctrl+Tab from inside a Settings page puts the keyboard on the first control of the page
  it reaches.** In build `1.0.0-alpha.1+g7d57cd49`, with focus on a control on the General
  tab, Ctrl+Tab reached the next page while it was still being built and left the keyboard on
  the empty page, so NVDA said "pane" and nothing else until Tab was pressed; arrowing along
  the tab row did not do this. Found from the source on 2026-09-17 while tracing #67 and
  filed as #68; it has not been heard on the alpha.1 build. A page reached from inside
  another page now puts focus on its first control, the order the build before alpha.1 had,
  and a page reached from the tab row leaves focus on the row as before. Measured from a test
  that builds the real dialog, focuses a General control, moves the selection the way
  Ctrl+Tab does once wxWidgets has read the key, and reads where the keyboard is: the first
  control of Compose, then of Reading; and still the row after an arrow on the row.
...
- **A sound scheme can be chosen, imported, and deleted under Settings,
  ...
  "Generated tones," the same tones as before, an Import sound scheme
  button next to it that reads a `.zip` a scheme's own creator built and
  shared, and a Delete sound scheme button beside that. A scheme can give
  some or all of the sixteen events a real sound file instead; anything it
  does not name still plays the built-in tone rather than nothing. The zip
  is checked before any of it is trusted: a size limit, no file allowed to
  write itself outside the scheme's own folder, every sound decoded for
  real rather than believed because of its file name, and a two-second
  limit on how long any one of them is allowed to run. Generated tones has
  no folder of its own and cannot be deleted, so the Delete button is
  disabled whenever it is the only scheme installed and refuses politely
  if somehow asked to remove it anyway; a machine can never end up with no
  scheme at all. A source for a first real scheme to try, "Soft Chimes,"
...
- **Sign In Again in the Account Manager now says what actually happened.**
  ...
  window, all before Windows was ever handed back control in between. NVDA
  read the application as unavailable instead of hearing either sentence,
  confirmed with a real screen reader run, both before this fix and after
  it. Sign In Again now answers its own button press directly, with the
  window staying open the whole time. Delete, Set as Default and Set Active
  on the same screen were changed the same way as a precaution, since they
  shared the exact code shape.

  Delete in the Calendar window, and in the Filter, Tag and Signature
  windows, were changed the same way too, and now have their own real
  screen reader tests proving they say what they should. What those tests
  do not show, honestly: deliberately reverting the fix and asking a real
  screen reader to press each button, twice, produced the correct
  announcement both times, not the silence or the "unavailable" Sign In
  Again's own bug caused. Whatever made Sign In Again's case fail this way
  did not reproduce here. The most likely difference is that Sign In Again
  gives two answers in a row from the same button press (what it is about
  to try, then what happened) where these give one; that was not chased
  down further, since the actual difference does not change what the right
  fix looks like, and closing the dialog around an announcement was never
  correct regardless of how reliably it went wrong. The Add/Edit Contact
  window's own Remove Email/Phone/Address/Custom Field buttons have the
  same code shape and are not changed by this entry.
...
- **The window that asks for a name when you create a calendar event,
  reminder, task or note, or rename one, now takes the Theme colours
  too.** The dialog and its name field now match the rest of the
  coloured application instead of showing Windows' own white regardless
  of what Light or Dark chose.
...
- Feedback preferences are per channel, not per event. The per-event overrides exist in the model and have no interface yet, because a grid of sixteen events by four channels is not the choice most people are making. Stronger than it reads: the setter is private, so no screen could offer one, and an override can only arrive by hand-editing the stored settings.
- Threading runs over the loaded folder rather than incrementally as mail arrives. The `References` headers are now stored by the sync, so conversations form from real mail; rethreading still happens when a folder is opened rather than as messages arrive. **Since closed:** mail arriving into an open folder now joins its thread without the folder being reopened, including the case where a late message merges two existing conversations.
