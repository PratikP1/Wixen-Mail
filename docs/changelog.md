...
- **The preview now has two independent ways out**, because inside a WebView one is not enough. `Escape` is handled by a listener injected into the document, and the first focusable thing on every preview page is a "Back to message list" button, so `Tab` reaches a real control before anything the sender wrote. They do not share a failure: the button works from the page alone.
